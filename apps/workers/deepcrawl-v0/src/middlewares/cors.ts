...
export const deepCrawlCors = createMiddleware<AppBindings>(async (c, next) => {
  ...
  const isTrusted = origin && [...ALLOWED_ORIGINS, ...DEVELOPMENT_ORIGINS].some((allowed) => {
    if (allowed.includes('*')) {
  ...
  return cors({
    ...
    allowHeaders: ['Content-Type', 'Authorization', 'X-API-Key', 'Accept', 'Origin', 'X-Requested-With', 'Cookie'],
