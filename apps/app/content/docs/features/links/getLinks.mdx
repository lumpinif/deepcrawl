...
`LinksSuccessResponse` is a union of two shapes.
  1. `LinksSuccessResponseWithTree` (when `tree` is enabled in options) – includes a `tree` hierarchy you can traverse, and metadata is nested in the tree node.
  2. `LinksSuccessResponseWithoutTree` (when `tree` is false in options) – omits `tree`, returning only extracted links and metadata.

<Callout type="info" title="LinksSuccessResponse">

- `LinksSuccessResponseWithTree`
  <AutoTypeTable
    path="../../packages/types/src/routers/links/types.ts"
    name="LinksSuccessResponseWithTree"
  />
- `LinksSuccessResponseWithoutTree`
  <AutoTypeTable
    path="../../packages/types/src/routers/links/types.ts"
    name="LinksSuccessResponseWithoutTree"
  />
