...
export async function readUrl(url: string) {
  const result = await deepcrawl.read.readUrl(url);
  // const result = await deepcrawl.read.readUrl(url, {...options}); // with more options
...
export async function extractLinks(url: string) {
  const result = await deepcrawl.links.extractLinks(url);
  // const tree = await deepcrawl.links.extractLinks(url, {...options}); // with more options
...
app.post('/api/read', async (req, res) => {
  try {
    const result = await deepcrawl.read.readUrl({
      url: req.body.url,
      metadata: true,
    });
    res.json(result);
