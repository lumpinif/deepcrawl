...
```text
apps/
  app/               Next.js dashboard + documentation
  workers/
    auth/            Cloudflare Worker handling auth flows
    v0/              Core API Worker (queue + fetch)
packages/
  ...
  sdks/js-ts/        TypeScript SDK exposed on npm
...
- **apps/app**: Next.js 15 app with the dashboard UI, docs, and API routes. Run
